/*
 * Original script by Canvas Dashboard Grades author.
 * https://chromewebstore.google.com/detail/canvas-dashboard-grades/jnlnibjgafmamocabdhidiaajkdphkpe
 */
      ...
      .then(function () {
        for (var i = 0, len = obj.length; i < len; i++) {
          var courseEnrollment =
            obj[i].enrollments[obj[i].enrollments.length - 1].computed_current_score;
        ...
        for (var b = 0, leng = courseBase.length; b < leng; b++) {
          var courseName =
            document.getElementsByClassName("ic-DashboardCard__link")[b].children[0].children[0]
              .title;
          for (var a = 0, lena = courseGradesID.length; a < lena; a++) {
            if (courseName.includes(courseGradesID[a]) == true) {
              ...
              canvasTitle.style.color = document.getElementsByClassName(
              ...
              document
                .getElementsByClassName("ic-DashboardCard__header_hero")
                [b].appendChild(canvasTitle);
            }
          }
        }
      });
